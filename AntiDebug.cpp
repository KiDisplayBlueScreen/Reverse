...
XAD_STATUS XAntiDebug::initialize()
{
	...
	if (_isX64)
	{
		...
		PIMAGE_SECTION_HEADER pSection = (PIMAGE_SECTION_HEADER)(sizeof(IMAGE_NT_SIGNATURE) + sizeof(IMAGE_FILE_HEADER) + pNtHead->FileHeader.SizeOfOptionalHeader + (ULONG_PTR)pNtHead);
...
BOOL XAntiDebug::IsDebuging()
{
	...
	if (_isX64) // 64bit
	{
...
#ifndef _WIN64
		status = X64Call((DWORD64)pfnSyscall64, 5,(DWORD64)-1, (DWORD64)0x1E, (DWORD64)&processInformation,(DWORD64)8,(DWORD64)&returnLength);
#else
		status = pfnSyscall64(
			(HANDLE)-1,
			(DWORD64)0x1E,
		...
		DWORD64		bugCheck;
#ifndef _WIN64
		status = X64Call((DWORD64)pfnSyscall64, 5,(DWORD64)-1,(DWORD64)0x1E,(DWORD64)&bugCheck,(DWORD64)8,(DWORD64)&bugCheck);
#else
		status = pfnSyscall64((HANDLE)-1,(DWORD64)0x1E,(PVOID)&bugCheck,(DWORD)8,(PDWORD64)&bugCheck);
#endif // !_WIN64
	...
	else // 32bit
	{
		...
		status = pfnSyscall32((HANDLE)-1,(DWORD)0x1E,&processInformation,(DWORD)4,&returnLength);
		...
		//利用内核二次覆盖的BUG来检测反调试，在利用这个漏洞之前计算一遍页面CRC
		DWORD	bugCheck;
		status = pfnSyscall32((HANDLE)-1,(DWORD)0x1E,&bugCheck,(DWORD)4,&bugCheck);
